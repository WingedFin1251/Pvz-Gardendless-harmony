...
> | `lite` | Gardendless Lite | `com.gardendless.lite` | 约 576 MB |
> | `gpnext` | **Gardendless GP-Next（本分支）** | `com.gardendless.gpnext` | 约 1.39 GB |
>
> 三条分支共享全部壳层源码，逐文件比对只有 5 个文件属于应用身份或说明
...
> 真机实测（MatePad 11.5"S / 8 GB RAM，冷启动后 **28 s 同时刻**取样，主进程 + 全部子进程）：
> `gpnext` 整组 PSS **1552.8 MB**（其中显存 530.5 MB），`lite` 为 **927.0 MB**（其中显存 156.0 MB）——
> **`lite` 省下约 626 MB**；而每 draw 提交成本两者相同（0.0894 ms vs 0.0895 ms），**`lite` 并不更快**。
...
| 缺失内容 | 说明 |
|:---|:---|
| `rawfile/assets/` | Cocos 资源包（本分支约 1.39 GB） |
...
1. 克隆本分支：`git clone -b gpnext https://github.com/WingedFin1251/Pvz-Gardendless-harmony.git`
...
未配置签名时产物为 `dist/pvzge-gpnext-<版本>.hap`（unsigned）；配置后会额外输出 `…-signed.hap`。
