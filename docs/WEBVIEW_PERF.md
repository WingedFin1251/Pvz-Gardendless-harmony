...
> 不要当成已证实的收益。
>
> 📊 **配套真机复测报告：[SHELL_PERF_2026-09-19.md](SHELL_PERF_2026-09-19.md)**
> （2026-09-19，MatePad 11.5"S：根因定位、假设排除、`lite`/`gpnext` 对照、改动验证；
> 原始输出归档在 `docs/perf/shell-ab-2026-09-19/`）。本文中标注「待实测」的条目
> 若已在该报告里给出数据，**以该报告为准**。
...
| 2026-09-19 | 新增配套真机复测报告 **[SHELL_PERF_2026-09-19.md](SHELL_PERF_2026-09-19.md)**（本节顶部已加指引）。要点：瓶颈确认为**页内逐帧计算**（约 28 ms/帧，GPU 最高频占比 0%、降分辨率与关 MSAA 后每 draw 成本不变）；`lite` 与 `gpnext` 每 draw 成本相同、但整组内存差约 620 MB；并记录了官方「App CPU 使用率」只覆盖应用主进程这一口径问题。原始输出归档至 `docs/perf/shell-ab-2026-09-19/`。**注：本行初版曾写「引擎帧率上限设为 60 比不设限快 21%」，该结论已被同日的 JS 归因复测推翻（交替 A/B 实测限帧反而慢 13.8%），详见该报告 §4.3** |
| 2026-09-19 | **新增游戏侧专文 [GAME_SIDE_OPTIMIZATION.md](GAME_SIDE_OPTIMIZATION.md)**：确认游戏 TypeScript 源码未公开（官网仓库只有构建产物），但负载自带 patcher 行为钩子；用「周期定时器成本排行榜」定位到 `hp-overlay` 每 **200 ms** 跑一次、每次约 16 ms、占主线程 **8.1% 墙钟**（其中仅约 1.7 ms 是 JS，其余是 Label 重绘 + 纹理上传）；稳态长任务 42 个 / 7.75 s / 44 s = **17.6% 墙钟**；JS 分配率仅 150–210 KB/s 且 `hp-overlay` 不在分配榜（"减少分配以降低 GC"不成立）。原始数据归档至 `docs/perf/game-side-2026-09-19/` |
