...
| 2026-09-19 | 新增配套真机复测报告 **[SHELL_PERF_2026-09-19.md](SHELL_PERF_2026-09-19.md)**（本节顶部已加指引）。要点：瓶颈确认为**页内逐帧计算**（约 28 ms/帧，GPU 最高频占比 0%、降分辨率与关 MSAA 后每 draw 成本不变）；`lite` 与 `gpnext` 每 draw 成本相同、但整组内存差约 620 MB；引擎帧率上限设为 60 比不设限快 21%；并记录了官方「App CPU 使用率」只覆盖应用主进程这一口径问题。原始输出归档至 `docs/perf/shell-ab-2026-09-19/` |
