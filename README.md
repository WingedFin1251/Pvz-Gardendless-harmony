...
> 原仓库 / 基础版 `Gardendless`（无 GP-Next）、以及精简版 `Gardendless-lite`（同一功能，仓库精简）。
...
| 原仓库 Release 的 HAP | 从原仓库 [Release](https://github.com/WingedFin1251/Pvz-Gardendless-harmony/releases) 页面下载。注意该 HAP 的包名 / 应用名是 `Gardendless`，与本应用的 `com.gardendless.gpnext` **不同**，因此两者可共存、但无法互相覆盖安装 |
